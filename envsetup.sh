...
function breakfast()
{
    target=$1
    local variant=$2
    BS_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/beanstalk/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var BS_VERSION)
    ...
    if (adb shell getprop ro.bs.device | grep -q "$BS_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $BS_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.bs.device | grep -q "$BS_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/beanstalk/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
