...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^spezi_") ; then
       CUSTOM_BUILD=$(echo -n $1 | sed -e 's/^spezi_//g')
    else
       CUSTOM_BUILD=
    fi
    export CUSTOM_BUILD
...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/spezi/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the spezi model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch spezi_$target-$variant
        fi
    fi
    return $?
}
